...
질문으로 학습자가 스스로 욕구(B, C)와 공통목표(A)를 발견하도록 돕는 인터랙티브
웹앱입니다.
...
워크북(TOC재단)의 단계 명칭을 따라 `Step 0~5`로 안내하며, Step 2~5에는 진행
스테퍼(필요→공통목표→가정→윈윈해결책)를 표시합니다.

0. **INTRO → PARTIES (Step 0)** — 누구와 누구의 갈등인지 당사자(`me`, `other`)를
   입력. 이 이름은 구름 다이어그램의 점선(D↔D') 위/아래와 이후 라벨·프롬프트에 반영됩니다.
1. **CONFLICT (Step 1)** — D(나의 요망사항/주장), D'(상대의 요망사항/주장) 입력
2. **NEEDS (Step 2 필요 찾기 → Step 3 공통목표 찾기)** — AI와 대화하며 B·C(필요)를
   도출하고("(필요)를 위해서는 (요망사항)을 해야만 한다"로 확인), 이어서 공통목표(A)를 제안.
   `SYSTEM_PROMPT`가 "정답을 먼저 말하지 않는다" 원칙을 강제하고, 대화 턴 수(`turnCount`)에
   따라 `[지시: ...]` 숨은 가이드를 주입합니다.
3. **GOAL (Step 3)** — 공통목표(A) 확정, 구름 다이어그램 완성
4. **ASSUMPTIONS (Step 4 가정 도출하기)** — 구름이 올바르게 작성됐는지 화살표를 문장으로 읽어 확인한 뒤,
   "B하기 위해서는 왜 꼭 D 해야만 할까? 왜냐하면~"으로 B–D, C–D′ 화살표에 숨은
   가정(전제)을 도출. `aPhase`(`bd` → `cd` → `done`) 상태머신으로 진행합니다.
5. **SOLUTIONS (Step 5 윈윈해결책 찾기)** — 도출한 가정에 도전해 윈윈 해결책(주입, injection)을 도출.
...
AI 응답에서 `👉 필요(B):`, `👉 필요(C):`, `🎯 공통목표(A):` 패턴을 정규식으로
